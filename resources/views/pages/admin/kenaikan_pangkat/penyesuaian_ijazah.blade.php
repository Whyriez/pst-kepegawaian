...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="h4 fw-bold text-primary mb-1">Penyesuaian Ijazah</h2>
            <p class="text-muted mb-0">Kelola pengajuan kenaikan pangkat jalur penyesuaian ijazah</p>
        </div>
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end border-0 shadow">
                <li><a class="dropdown-item" href="{{ route('admin.kp.penyesuaian_ijazah') }}">Semua</a></li>
                <li><a class="dropdown-item"
                        href="{{ route('admin.kp.penyesuaian_ijazah', ['status' => 'pending']) }}">Menunggu</a></li>
                <li><a class="dropdown-item"
                        href="{{ route('admin.kp.penyesuaian_ijazah', ['status' => 'disetujui']) }}">Disetujui</a></li>
                <li><a class="dropdown-item"
                        href="{{ route('admin.kp.penyesuaian_ijazah', ['status' => 'ditunda']) }}">Ditunda</a></li>
                <li><a class="dropdown-item"
                        href="{{ route('admin.kp.penyesuaian_ijazah', ['status' => 'ditolak']) }}">Ditolak</a></li>
            </ul>
        </div>
    </div>

    {{-- Statistik Cards (Sama seperti Fungsional) --}}
    <div class="row g-3 mb-4">
        <div class="col-md-3">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body d-flex align-items-center">
                    <div class="bg-primary-subtle text-primary rounded-circle p-3 me-3"><i
                            class="fas fa-graduation-cap fa-lg"></i></div>
                    <div>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light">
                        <tr>
                            ...
                            <th class="py-3">Jabatan & Unit</th> {{-- Disesuaikan --}}
                    ...
                    <tbody>
                        @forelse($pengajuan as $item)
                            <tr>
                                ...
                                {{-- 3. Info Tambahan (Jabatan/Unit/Pendidikan Baru) --}}
                                <td>
                                    <div class="small fw-bold">{{ $item->pegawai->jabatan }}</div>
                                    {{-- Jika ada data pendidikan baru di JSON, tampilkan. Jika tidak, tampilkan unit kerja --}}
                                    <small class="text-muted">
                                        {{ $item->data_tambahan['pendidikan_baru'] ?? ($item->pegawai->satuanKerja->nama_satuan_kerja ?? 'Unit Kerja') }}
                                    </small>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5 text-muted">
                                    ...
                                    <p>Belum ada pengajuan penyesuaian ijazah.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="modal fade" id="rejectModalPage" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('admin.kp.penyesuaian_ijazah.reject') }}" method="POST" id="formTolak">
                    @csrf
                    <div class="modal-header bg-danger text-white">
                        <h5 class="modal-title">Tolak Pengajuan PI</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="kategori" required>
                                <option value="ijazah_tidak_valid">Ijazah Tidak Valid</option>
    ...
    <div class="modal fade" id="postponeModalPage" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('admin.kp.penyesuaian_ijazah.postpone') }}" method="POST" id="formTunda">
                    @csrf
                    <div class="modal-header bg-warning text-dark">
                        <h5 class="modal-title">Tunda Pengajuan PI</h5>
    ...
    {{-- FORM APPROVE HIDDEN --}}
    <form id="formApproveHidden" action="{{ route('admin.kp.penyesuaian_ijazah.approve') }}" method="POST"
        style="display: none;">
        @csrf
...
@push('scripts')
    <script>
        ...
        function confirmApprove(id) {
            Swal.fire({
                title: 'Setujui PI?',
                text: "Pastikan ijazah sudah diverifikasi.",
                icon: 'question',
