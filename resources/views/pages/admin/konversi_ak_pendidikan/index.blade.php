...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="h4 fw-bold text-primary mb-1">Konversi AK Pendidikan</h2>
            <p class="text-muted mb-0">Verifikasi usulan konversi angka kredit pendidikan pegawai</p>
        </div>
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end border-0 shadow">
                <li><a class="dropdown-item" href="{{ route('admin.konversi_ak_pendidikan') }}">Semua</a></li>
                <li><a class="dropdown-item" href="{{ route('admin.konversi_ak_pendidikan', ['status' => 'pending']) }}">Menunggu Verifikasi</a></li>
                <li><a class="dropdown-item" href="{{ route('admin.konversi_ak_pendidikan', ['status' => 'disetujui']) }}">Disetujui</a></li>
                <li><a class="dropdown-item" href="{{ route('admin.konversi_ak_pendidikan', ['status' => 'ditunda']) }}">Ditunda</a></li>
                <li><a class="dropdown-item" href="{{ route('admin.konversi_ak_pendidikan', ['status' => 'ditolak']) }}">Ditolak</a></li>
            </ul>
        </div>
    </div>
                    ...
                    @forelse($pengajuan as $item)
                        <tr>
                            ...
                            <td>
                                ...
                                <small class="text-muted">{{ $item->data_tambahan['pangkat'] ?? '-' }} ({{ $item->data_tambahan['golongan_ruang'] ?? '-' }})</small>
                            </td>
                            <td>
                                <div class="small text-muted">
                                    {{ $item->data_tambahan['satuan_kerja'] ?? '-' }}
                                </div>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                @php
                                    $berkasList = $item->dokumenPengajuans->map(function ($doc) {
                                        ...
                                        // Cek ekstensi manual untuk icon
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-5 text-muted">
                                ...
                                <p>Belum ada pengajuan Konversi AK Pendidikan.</p>
                            </td>
                        </tr>
                    @endforelse
                    </tbody>
    ...
    <div class="modal fade" id="rejectModalPage" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('admin.konversi_ak_pendidikan.reject') }}" method="POST" id="formTolak">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="kategori" required>
                                ...
                                <option value="dokumen_tidak_valid">Dokumen Tidak Valid/Rusak</option>
    ...
    <div class="modal fade" id="postponeModalPage" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('admin.konversi_ak_pendidikan.postpone') }}" method="POST" id="formTunda">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="alasan" rows="3" required
                                      placeholder="Menunggu verifikasi lanjutan..."></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Batal</button>
    ...
    {{-- MODAL PREVIEW (REQUIRED FOR JS) --}}
    <div class="modal fade" id="previewModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content h-100">
                <div class="modal-header py-2">
                    <h5 class="modal-title fs-6 fw-bold" id="previewTitle">Preview Dokumen</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-0">
                    <div class="d-flex h-100">
                        <div class="bg-light border-end" style="width: 280px; min-width: 280px;">
                            <div class="list-group list-group-flush" id="fileListContainer">
                            </div>
                        </div>
                        <div class="flex-grow-1 bg-secondary bg-opacity-10 position-relative d-flex align-items-center justify-content-center p-3" style="min-height: 500px;">

                            <iframe id="pdfViewer" src="" class="w-100 h-100 rounded shadow-sm border" style="display: none;"></iframe>

                            <img id="imageViewer" src="" class="img-fluid rounded shadow-sm" style="display: none; max-height: 100%; max-width: 100%; object-fit: contain;">

                            <a id="btnDownloadFile" href="#" target="_blank" class="btn btn-primary position-absolute bottom-0 end-0 m-3 shadow-sm">
    ...
    {{-- HIDDEN APPROVE FORM --}}
    <form id="formApproveHidden" action="{{ route('admin.konversi_ak_pendidikan.approve') }}" method="POST"
          style="display: none;">
        @csrf
...
@push('scripts')
    <script>
        ...
        function loadPreview(element) {
            ...
            if (files.length === 0) {
                Swal.fire('Info', 'Tidak ada dokumen untuk ditampilkan', 'info');
                return;
            }
            ...
            if (listContainer) {
                ...
                files.forEach((file, index) => {
                    ...
                    item.href = '#';
                    item.className = `list-group-item list-group-item-action p-3 border-bottom ${index === 0 ? 'active' : ''}`;
                    item.innerHTML = `
                        <div class="d-flex w-100 justify-content-between align-items-center">
                            <div class="d-flex align-items-center gap-2 overflow-hidden">
                                <i class="fas ${icon} ${color}"></i>
                                <span class="text-truncate fw-medium" style="max-width: 180px;">${file.nama_dokumen}</span>
                            </div>
                        </div>`;
                    item.onclick = (e) => {
                        e.preventDefault();
        ...
        function showFileInViewer(file) {
            ...
            if (file.is_pdf) {
                if (pdfViewer) {
                    // Clone to force refresh
                    const clone = pdfViewer.cloneNode(true);
                    clone.setAttribute('src', file.url + '#toolbar=0');
