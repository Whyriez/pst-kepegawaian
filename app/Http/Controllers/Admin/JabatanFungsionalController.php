...
{
    public function naikJenjang(Request $request)
    {
        // 1. Ambil ID Layanan
        $layanan = JenisLayanan::where('slug', 'jf-naik-jenjang')->first();

        if (!$layanan) {
            return back()->with('error', 'Layanan Kenaikan Jenjang JF belum dikonfigurasi.');
        }
        ...
        return view('pages.admin.jabatan_fungsional.naik_jenjang', compact('pengajuan', 'stats'));
    }

    public function pemberhentian(Request $request)
    {
        // 1. Ambil ID Layanan
        $layanan = JenisLayanan::where('slug', 'jf-pemberhentian')->first();

        if (!$layanan) {
            return back()->with('error', 'Layanan Pemberhentian JF belum dikonfigurasi.');
        }
        ...
        return view('pages.admin.jabatan_fungsional.pemberhentian', compact('pengajuan', 'stats'));
    }

    public function pengangkatan(Request $request)
    {
        // 1. Ambil ID Layanan
        $layanan = JenisLayanan::where('slug', 'jf-pengangkatan')->first();

        if (!$layanan) {
            return back()->with('error', 'Layanan Pengangkatan JF belum dikonfigurasi.');
        }
        ...
        return view('pages.admin.jabatan_fungsional.pengangkatan', compact('pengajuan', 'stats'));
    }
