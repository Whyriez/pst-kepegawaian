...
class SatyalancanaController extends Controller
{
    public function index()
    {
        ...
        $pengajuans = Pengajuan::with('jenisLayanan', 'pegawai')
            ...
            ->whereHas('jenisLayanan', function ($query) {
                $query->where('slug', 'satyalancana');
            })
            ->latest()
            ->paginate(10);

        return view('pages.user.satyalancana.index', compact('pengajuans'));
    }
    ...
    public function create()
    {
        ...
        return view('pages.user.satyalancana.create', compact('pegawai', 'syarat'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('satyalancana')->with('success', 'Pengajuan Satyalancana berhasil dikirim! Tiket: ' . $tiket);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function edit(Request $request)
    {
        ...
        if (!$id) {
            return redirect()->route('satyalancana')->with('error', 'ID Pengajuan tidak ditemukan.');
        }
        ...
        return view('pages.user.satyalancana.edit', compact('pengajuan', 'syarat'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $dataTambahan['jabatan'] = $request->jabatan_satyalancana;
            $dataTambahan['pangkat'] = $request->pangkat_satyalancana;
            $dataTambahan['unit_kerja'] = $request->unit_kerja_satyalancana;
            $dataTambahan['golongan_ruang'] = $request->golongan_ruang_satyalancana;
            ...
            DB::commit();
            return redirect()->route('satyalancana')->with('success', 'Perbaikan data Satyalancana berhasil dikirim!');
